...
<body>
	...
	<div class="container">
		...
		<table class="table table-hover">
			<thead>
				...
				<tr>
					<td>이번달 총매출</td>
					<td><fmt:formatNumber value="${map.SUMPAY}" pattern="#,###"/>원</td>
					<td>오늘 매출</td>
					<td><fmt:formatNumber value="${map.DAYPAY}" pattern="#,###"/>원</td>
				</tr>
			</thead>
		</table>
	</div>
	...
	<script>
		...
		var json = new Object();
		peoples.push("${mpp.get(i)}");
		</c:forEach>

		var prices = new Array();
		<c:forEach var="i"  begin="1" end="31" varStatus="status">
		var json = new Object();
		prices.push("${mp.get(i)}");
		</c:forEach>
			...
			<tr>
				<c:forEach var="i" begin="1" end="31" varStatus="status">
					<td style="padding: 4px; word-break: keep-all;"><c:if
							test="${mpp.get(i) eq null}">0</c:if> ${mpp.get(i)}명</td>
				</c:forEach>
			</tr>
		</table>
	</div>
